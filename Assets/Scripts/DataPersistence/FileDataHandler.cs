...
public class FileDataHandler 
{
    ...
    private readonly string encryptionCodeWord = "4WHcviQUhvw9bco";
    ...
    public GameData Load(string profileId, bool allowRestoreFromBackup = true) 
    {
        ...
        if (File.Exists(fullPath)) 
        {
            ...
            {
                // since we're calling Load() recursively, we need to account for the case where the rollback succeeds, but data is still 
                // failing to Load for some other reason, which without this check may cause an infinite recursion loop
    ...
    public void Save(GameData data, string profileId) 
    {
        ...
        try 
        {
            ...
            GameData verfiedGameData = Load(profileId);
            // if the data can be verified, back it up
            if(verfiedGameData != null)
            {
                File.Copy(fullPath, backupFilePath, true);
            }
            // otherwise, something went wrong and we should throw an exeption
            else
            {
